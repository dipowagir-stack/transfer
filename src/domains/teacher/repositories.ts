import { collection, doc, getDocs, getDoc, query, where, setDoc, addDoc, updateDoc, deleteDoc, writeBatch, serverTimestamp, orderBy } from 'firebase/firestore';
...
export class CustomTeacherRepository {
  async getByQueryDesc(coll: string, field: string, operator: any, value: any, orderByField: string) {
    if (virtualDatabase.isActive()) {
      const items = await virtualDatabase.getDocs(coll, (item) => {
        if (operator === '==' || operator === '===') return item[field] === value;
        if (operator === '!=') return item[field] !== value;
        if (operator === 'in') return Array.isArray(value) && value.includes(item[field]);
        return true;
      });
      return items.sort((a, b) => (b[orderByField] || '').localeCompare(a[orderByField] || ''));
    }
    const q = query(collection(db, coll), where(field, operator, value), orderBy(orderByField, 'desc'));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
  }

  async getAllDesc(coll: string, orderByField: string) {
    if (virtualDatabase.isActive()) {
      const items = await virtualDatabase.getDocs(coll);
      return items.sort((a, b) => (b[orderByField] || '').localeCompare(a[orderByField] || ''));
    }
    const q = query(collection(db, coll), orderBy(orderByField, 'desc'));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
  }

  async submitAttendanceBatch(logColl: string, attendanceData: any) {
    if (virtualDatabase.isActive()) {
      const logId = await virtualDatabase.addDoc(logColl, {
        ...attendanceData.log,
        createdAt: new Date().toISOString()
      });

      for (const student of attendanceData.studentsToUpdate) {
        const studentDoc = await virtualDatabase.getDoc('users', student.uid);
        if (studentDoc) {
          if (student.pointsToAdd !== undefined) {
            await virtualDatabase.updateDoc('users', student.uid, {
              points: (studentDoc.points || student.points || 0) + student.pointsToAdd
            });
          } else {
            await virtualDatabase.updateDoc('users', student.uid, {
              absences: (studentDoc.absences || student.absences || 0) + 1
            });
          }
        }
      }
    ...
    const logRef = doc(collection(db, logColl));
    ...
    for (const student of attendanceData.studentsToUpdate) {
      ...
      if (student.pointsToAdd !== undefined) {
         batch.update(studentRef, {
           points: student.points + student.pointsToAdd
         });
      } else {
         batch.update(studentRef, {
...
export const customRepo = new CustomTeacherRepository();
